...
pub fn success_alert(props: &Props) -> Html {
    if let Some(message) = &props.alert_msg {
        if !message.is_empty() {
            html! {
                <div class="fixed top-2 right-2 inline-flex w-1/3 items-center rounded-lg bg-green-300 px-6 py-5 text-base text-green-700" role="alert">
                    <span class="mr-2">
...
pub fn danger_alert(props: &DangerProps) -> Html {
    if let Some(error) = &props.error {
        html! {
            <div class="fixed top-2 right-2 inline-flex w-1/3 items-center rounded-lg bg-red-300 px-6 py-5 text-base text-red-700" role="alert">
                <span class="mr-2">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
...
pub fn warning_alert(props: &Props) -> Html {
    if let Some(message) = &props.alert_msg {
        html! {
            <div class="fixed top-2 right-2 inline-flex w-1/3 items-center rounded-lg bg-yellow-300 px-6 py-5 text-base text-yellow-800" role="alert">
                <span class="mr-2">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
